...
fish <- read_csv(
  here("Week 7", "fish_data", "BlackfootFish.csv"),
...
missing_summary <- fish |>
  summarise(
    rows_with_missing = sum(!complete.cases(.)),
...
fish |>
  pivot_longer(
    cols = c(length, weight, mark, section, species),
    names_to = "variable",
    values_to = "value"
  ) |>
  mutate(is_missing = is.na(value)) |>
  group_by(year, trip, variable) |>
  summarise(prop_missing = mean(is_missing), .groups = "drop") |>
  ggplot(aes(x = year, y = prop_missing, color = variable)) +
  geom_line() +
  facet_wrap(~trip) +
  ...
  labs(
    ...
    y = "Percent Missing",
    color = "Variable",
    title = "Missingness by Year and Trip"
  )
```
...
rescale_01 <- function(x) {
  if (!is.numeric(x)) {
    stop("rescale_01() requires a numeric vector.")
  }
  if (length(x) <= 1) {
    stop("rescale_01() needs a vector with length > 1.")
  }
...
fish |>
  ...
  geom_histogram(binwidth = 0.05, fill = "#5B8FF9") +
  labs(x = "Rescaled Length (0-1)") +
  scale_y_continuous(limits = c(0,4000))
