...
ggplot(data = surveys,
       mapping = aes(x = species, y = weight)) +
  geom_boxplot(outlier.shape = NA)  +
   geom_jitter(color = "steelblue", alpha = .5)
  labs(
    title = "Distribution of Rodent Weights by Species",
    x = "Species",
    y = "Weight (grams)"
...
ggplot(data = surveys,
  ...
  geom_boxplot(outlier.shape = NA) +
  geom_jitter(color = "steelblue", alpha = 0.5, height = 0.2) +
  labs(
    title = "Distribution of Rodent Weights by Species (Flipped)",
    x = "Weight (grams)",
    y = "Species"
  ) +
  theme_minimal()
...
Based on the ANOVA results, there is strong evidence that rodent species differ in their mean weights. The F-test was significant (F(13, 30449) = 19916, p < 2e-16). Therefore, we reject the null hypothesis and conclude that at least one species has a different mean weight from the others.
