...
#| label: explore-data
dim(teacher_evals)
n_distinct(teacher_evals$teacher_id)
n_distinct(teacher_evals$course_id)
summary(teacher_evals$SET_score_avg)
```
...
summary(instructors$seniority)

```
Most instructors hold either doctoral or professor titles, though a smaller number have master’s degrees. The gender distribution is fairly balanced, and seniority ranges from early-career instructors (1–3 years) to long-tenured professors with over 30 years of experience. This variety allows comparisons across different instructor backgrounds.
...
q1_summary <- teacher_evals_clean |>
  filter(question_no == 901) |>
  group_by(teacher_id) |>
  summarise(mean_q1 = mean(SET_score_avg, na.rm = TRUE), n_courses = n()) |>
  filter(n_courses >= 5)

q1_summary |>
  summarise(
    highest_q1 = max(mean_q1),
    lowest_q1 = min(mean_q1)
  )
...
deg_resp |>
  summarise(
    highest_response = max(avg_resp),
    lowest_response = min(avg_resp)
  )
